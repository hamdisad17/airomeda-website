...
export const alt = 'Airomeda';
export const size = { width: 1200, height: 630 };
export const contentType = 'image/png';

export default async function OG({ params }: { params: Promise<{ locale: 'tr' | 'en' }> }) {
  const { locale } = await params;
  const subtitle = SITE.description[locale];
    ...
    (
      ...
      >
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 24 }}>
          ...
          >
            {locale === 'tr' ? 'Karmaşık olanı, basit kıl.' : 'Make the complex, simple.'}
          </div>
          <div style={{ fontSize: 28, color: '#cbd5e1', maxWidth: 1000, display: 'flex' }}>
            {subtitle}
          </div>
        </div>
